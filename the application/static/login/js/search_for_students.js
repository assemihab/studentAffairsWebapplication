  ...
  if (url.split("/")[3] == "act-in.html")
  {
    ...
    for (const student of students.slice(1))
    {
      if (student[9] == "active")
      {
        ...
        cell1.innerHTML = '<input name = "activeR" type="radio" id="' + student[0] + '"/>';
    ...
    for (const student of students.slice(1))
    {
      if (student[9] == "inactive")
      {
        ...
        cell1.innerHTML = '<input name = "inactiveR" type="radio" id="' + student[0] + '"/>';
        cell2.innerHTML = '<td>' + student[1] + '</a></td>';
        cell3.innerHTML = '<td>' + student[0] + '</a></td>';
        cell4.innerHTML = '<td>' + student[6] + '</a></td>';
        cell5.innerHTML = '<td>' + student[9] + '</a></td>';
...
export function SearchFor() {
  ...
  for (const student of students.slice(1))
  {
    if (student[1] === name && student[9] == "active")
    {
      ...
      cell4.innerHTML = '<button id="update" name="'+ student[0] +'" onclick="sendData(this.id, this.name)">Update Profile</button></a>';
      cell5.innerHTML = '<button id="updateDept" name="'+ student[0] +'"  onclick="sendData(this.id, this.name)">Assign Department</button></a>';
    }
  }
